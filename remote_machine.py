...
async def stream_handler(ws, path):
    """Send desktop frames to the connected client."""
    client = id(ws)
...
async def control_handler(ws, path):
    """Receive mouse/keyboard events from the client and execute them."""
    client = id(ws)
...
async def handler(ws):
    ...
    if path == "/video":
        await stream_handler(ws, path)
    elif path == "/control":
        await control_handler(ws, path)
    else:
...
async def run_server(bind_ip, port):
    ...
    server = await websockets.serve(
        handler,
        bind_ip, port, ssl=ssl_ctx
    )
    print(f"Server listening on wss://{bind_ip}:{port}")
